...
//
// This package converts raw audio samples into a spectrogram - a time-frequency
// representation that shows how frequency content changes over time.
//
// How Spectrogram Generation Works:
//
// 1. Preprocessing:
//    - Low-pass filter: Remove frequencies above 5kHz (reduces noise, focuses on relevant range)
//    - Downsampling: Reduce sample rate by factor of 4 (reduces computation, keeps relevant frequencies)
//
// 2. Short-Time Fourier Transform (STFT):
//    - Divide audio into overlapping windows (hop size = window_size / 32)
//    - Window size: 1024 samples (freqBinSize)
//    - Apply Hamming window to each segment (reduces spectral leakage)
//    - Compute FFT for each window to get frequency content
//
// 3. Window Function:
//    - Hamming window: w(n) = 0.54 - 0.46*cos(2πn/N)
//    - Tapers signal at edges to prevent discontinuities
//    - Reduces artifacts in frequency domain
//
// 4. Result:
//    - Spectrogram: 2D array [time_window][frequency_bin]
//    - Each cell contains complex number representing magnitude and phase
//    - Used for peak extraction and fingerprinting
//
// The spectrogram is the foundation for all audio analysis - it reveals the
// frequency patterns that make drone propellers identifiable, including:
// - Rotor blade passing frequencies (harmonics)
// - Motor RPM signatures
// - Propeller resonance frequencies
